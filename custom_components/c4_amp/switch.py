...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for entity_key, data in hass.data[DOMAIN].items():
        conf = data["config"]
        state = data["state"]
        entities.append(C4ZonePowerSwitch(entity_key, conf, state, data))
...
class C4ZonePowerSwitch(SwitchEntity, RestoreEntity):
    ...
    async def _ping(self) -> bool | None:
        ...
        if self._state_ref.get("power") and self._sources:
            ...
            return await amp_channel_on(self._channel, int(first_source_id), self._ip, self._port, cancel=cancel)
    ...
    async def async_turn_on(self, **kwargs):
        if self._sources:
            first_source_id = list(self._sources.keys())[0]
            cancel = cancel_and_replace(self._data)
            result = await amp_channel_on(self._channel, int(first_source_id), self._ip, self._port, cancel=cancel)
            if result is None:
                return
            self._handle_result(result)
            if result:
